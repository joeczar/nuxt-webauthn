...
export function useWebAuthnRegistration() {
    ...
    async function registerWebAuthn(email: string) {
        ...
        try {
            ...
            const options = await $fetch<PublicKeyCredentialCreationOptionsJSON>('/api/auth/register-webauthn', {
                method: 'POST',
                body: { email }
            })
            ...
            const verificationResponse = await $fetch<{ success: boolean; message: string }>('/api/auth/register-webauthn', {
                method: 'POST',
